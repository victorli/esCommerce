{*
/**
 * @license MIT
 * @author BLX90<zs.li@blx90.com>
 * @since 2014
 */
 *}
 {if $is_splash}
 	<div class="box">
 		<h2 class="page-subheading">{l s='Login alipay to pay' mod='alipay'}</h2>
 		<div class="dark">{l s='Choose after paid at new opened window:' mod='alipay'}</div>
 		<ul>
 			<li>
 				<div>
 					<span class="icon-ok"></span>
 					<span>{l s='Paid successfully' mod='alipay'}</span>
 					<span>| {l s='You can choose:' mod='alipay'}</span>
 					<a href="index.php?controller=history" >{l s="View orders" mod='alipay'}</a>
 				</div>
 			</li>
 			<li>
 				<div>
 					<span class="icon-remove"></span>
 					<span>{l s='Fail to pay' mod='alipay'}</span>
 					<span>| {l s='Suggest your choices:' mod='alipay'}</span>
 					<a href="" >{l s="Choose other payment" mod='alipay'}</a>
 				</div>
 			</li>
 		</ul>
 	</div>
 {else}
	{capture name=path}{l s='Order confirmation'}{/capture}
	
	<h1 class="page-heading">{l s='Order confirmation'}</h1>
	
	{assign var='current_step' value='payment'}
	{include file="$tpl_dir./order-steps.tpl"}
	
	{include file="$tpl_dir./errors.tpl"}
	
	{if $is_guest}
		<p>{l s='Your order ID is:'} <span class="bold">{$id_order_formatted}</span> . {l s='Your order ID has been sent via email.'}</p>
	    <p class="cart_navigation exclusive">
		<a class="button-exclusive btn btn-default" href="{$link->getPageLink('guest-tracking', true, NULL, "id_order={$reference_order}&email={$email}")|escape:'html':'UTF-8'}" title="{l s='Follow my order'}"><i class="icon-chevron-left"></i>{l s='Follow my order'}</a>
	    </p>
	{else}
	<p class="cart_navigation exclusive">
		<a class="button-exclusive btn btn-default" href="{$link->getPageLink('history', true)|escape:'html':'UTF-8'}" title="{l s='Back to orders'}"><i class="icon-chevron-left"></i>{l s='Back to orders'}</a>
	</p>
	 <form action="{$action}_input_charset={$input_charset}" method="post" id="blx_alipay_jump_form">
	 	{foreach from=$inputs key=key item=param}
	 		<input type="hidden" name="{$key}" value="{$param}" />
	 	{/foreach}
	 </form>
	 <script type="text/javascript">
		$(document).ready(function(e){
			$('form#blx_alipay_jump_form').submit();
		});
	</script>
	{/if}
{/if}